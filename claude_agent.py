    ...
    async def generate_typescript(args):
        ...
        try:
            ...
            if crud_scaffolds:
                ...
                response_text += f"\n{'='*60}"
                response_text += "\n"
                response_text += "\n1. DO NOT USE Read TOOL: ALL files you need are included below IN FULL."
                response_text += "\n   index.css, Layout.tsx, types, service, App.tsx, DashboardOverview — ALL included."
                response_text += "\n   Do NOT Read ANY of these files. Do NOT Read CRUD pages, dialogs, or scaffold components."
                response_text += "\n"
                response_text += "\n2. WRITE/EDIT EACH FILE EXACTLY ONCE — NO RE-WRITES!"
                response_text += "\n   After you Write or Edit a file, it is DONE. Move to the next file."
                response_text += "\n   Do NOT Read a file back after writing it. Do NOT rewrite a file you already wrote."
                response_text += "\n   If you spot a small issue later, use Edit with a SMALL old_string/new_string fix."
                response_text += "\n   NEVER use Write to replace an entire file you just wrote — use Edit for fixes."
                response_text += "\n"
                ...
                response_text += "\n"
                response_text += "\n4. DashboardOverview.tsx: Write it ONCE. Plan the ENTIRE component in your head first:"
                response_text += "\n   - Decide ALL imports before writing (no unused imports!)"
                response_text += "\n   - Decide the full layout (hero, KPIs, charts, table)"
                response_text += "\n   - Then Write ONCE. If a small fix is needed later, use Edit — NEVER rewrite the whole file."
                response_text += "\n"
                response_text += "\nSTEPS (in order):"
            ...
            if crud_scaffolds:
                ...
                # Layout.tsx: include FULL file so agent never needs to Read it
                layout_path = Path("src/components/Layout.tsx")
                if layout_path.exists():
                    response_text += f"\n\n{'='*60}\n📄 src/components/Layout.tsx (FULL — do NOT Read this file, edit APP_TITLE/APP_SUBTITLE directly)\n{'='*60}\n{layout_path.read_text()}"
            
            # Include FULL index.css so agent never needs to Read it
            css_path = Path("src/index.css")
            if css_path.exists():
                response_text += f"\n\n{'='*60}\n📄 src/index.css (FULL — use Edit to EXTEND :root, do NOT Read or Write this file)\n{'='*60}\n{css_path.read_text()}"
